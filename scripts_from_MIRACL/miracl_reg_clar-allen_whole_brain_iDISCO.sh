...
if [[ -z ${lbls} ]]; then
    ...
    lbls=${atlasdir}/ara/gubra/gubra_ano_${hemi}_${vox}um.nii.gz ### Changed atlas
...
function warpallenlbls()
{
    ...
    ###ifdsntexistrun ${tiflbls} "Un-padding high res tif" c3d ${unpadtif} -pad -10% -10% -o ${tiflbls} ###uncomment to make low res tif
...
###    ifdsntexistrun ${lblsorgnii} "Extracting labels to original size" \
   ### c3d ${orgortlbls} -region ${xr}x${yr}x${zr} ${alldim} ${lblsorgnii} ### commented out by DRR on 08/03/22

   ### c3d ${inclar} ${lblsorgnii} -copy-transform -o ${lblsorgnii} ### commented out by DRR on 08/03/22
...
# 2a) initialize registration
    ...
    if [[ "${bulb}" == 0 ]]; then
        allenref=${atlasdir}/ara/gubra/gubra_template_wo_OB_25um${side}.nii.gz ### Changed average template and removed olfactory bulb

    elif [[ "${bulb}" == 1 ]]; then
        allenref=${atlasdir}/ara/gubra/gubra_template_25um${side}.nii.gz ### Changed average template
...
# 4) Warp input CLARITY to Allen
	...
	# hres Allen
	allenhres=${atlasdir}/ara/gubra/gubra_template_10um.nii.gz ### changed average template
	###allenhres=${atlasdir}/ara/gubra/gubra_ano_split_10um.nii.gz #(fixed_labels_22_02_28)
