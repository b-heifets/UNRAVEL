...
def parse_args():
    ...
    reqs.add_argument('-i', '--input', help='Path to a NIfTI mask. All values > 0 are treated as foreground.', required=True, action=SM)
...
def main():
    ...
    # Treat all positive voxels as foreground
    img_bin = load_mask(args.input) > 0

    img_dil = binary_dilation(
        img_bin,
