...
Features:
---------
- Loads all matching CSV files (glob pattern).
...
def parse_args():
    ...
    opts.add_argument('-i', '--input', help='Glob pattern for input CSV files.', required=True, action=SM)
...
def main():
    ...
    input_files = list(Path().cwd().glob(args.input))
    if not input_files:
