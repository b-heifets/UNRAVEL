...
"""
Use ``mixedlm_csv.py`` from UNRAVEL to generate a CSV for smf.mixedlm() analysis, containing region-wise intensities.
...
def parse_args():
    ...
    reqs.add_argument('-y', '--y_img_glob', help='path/y_axis_image_*.nii.gz (cFos maps).', required=True, action=SM)
...
def extract_regional_intensities(x_img_path, imgY, atlas_img, mask_img):
    ...
    try:
        ...
        # Create dataframe
        gene = Path(x_img_path).stem.split('_')[0]  # Extract gene name
        df = pd.DataFrame({'RegionID': list(imgX_means.keys()), gene: list(imgX_means.values()), 'cFos': list(imgY_means.values())})
        return df
...
def main():
    ...
    with Progress(TextColumn("[bold blue]{task.description}"), BarColumn(), "[progress.percentage]{task.percentage:>3.0f}%") as progress:
        ...
        for y_img_path in y_img_paths:
            ...
            group = Path(y_img_path).stem.split('_')[0]
            mouse_id = Path(y_img_path).stem.split('_')[1]

            # Process X images for this Y image
            futures = []
            with ThreadPoolExecutor() as executor:
                for x_img_path in x_img_paths:
                    futures.append(
                        executor.submit(extract_regional_intensities, x_img_path, imgY, atlas_img, mask_img)
                    )
    ...
    # Sort columns alphabetically after the first four
    ...
    if failed_tasks:
        print(f"\n    [yellow]Warning: {len(failed_tasks)} tasks failed. See details below:")
