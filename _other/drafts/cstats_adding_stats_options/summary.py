...
def main():
    ...
    for subdir in [d for d in Path.cwd().iterdir() if d.is_dir() and d.name not in ('3D_brains', 'valid_clusters_tables_and_legend')]:
        ...
        # Detect which test type result file exists
        test_types = ['t-test', 'holm', 'dunnett', 'tukey', 'anova']
        results_file = None
        for tag in test_types:
            results_file = next(stats_output.glob(f'{tag}_results.csv'), None)
            if results_file:
                test_type = tag
                break

        if results_file is None:
            print(f"[yellow]No *_results.csv found in {stats_output}, skipping...[/]")
            continue

        if args.verbose:
            print(f"[bold cyan]Detected test type:[/] {test_type} [dim]in {subdir.name}[/]")

        valid_clusters_ids_txt = stats_output / f'valid_cluster_IDs_{test_type}.txt'
        if not valid_clusters_ids_txt.exists():
            print(f"[yellow]No valid_cluster_IDs_{test_type}.txt found in {stats_output}, skipping...[/]")
            continue
        ...
        if not rev_cluster_index_path.exists():
            rev_cluster_index_path = next(subdir.glob("*rev_cluster_index*"), None)
