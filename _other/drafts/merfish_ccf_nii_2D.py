...
def parse_args():
    ...
    reqs.add_argument('-z', '--z_index', help='Z index to select slice', required=True, type=int, action=SM)
...
def main():
    ...
    # Filter for the selected z slice
    z_value = full_data['z_ccf'].unique()[args.z_index]
    print(f"Using Z slice {args.z_index} (Z coordinate: {z_value} µm)")
    ...
    # Generate the image slice with 10-micron resolution
    ...
    if args.output:
        ...
        output_path = root_dir / f"{args.gene}_z{z_value:.1f}um_slice.nii.gz"
        
    nii_img = nib.Nifti1Image(img_slice, affine=affine)
    nib.save(nii_img, str(output_path))
    print(f"\nSaved image slice to {output_path}\n")
