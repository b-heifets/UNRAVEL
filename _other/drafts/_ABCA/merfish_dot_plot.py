...
def parse_args():
    ...
    opts.add_argument('-r', '--region_level', help='Region level (category, division [default], structure, substructure). It will be prepended with parcellation_', default='division', action=SM)
...
def main():
    ...
    for cell_type in cell_types:
        ...
        cell_exp_df = cell_exp_df[cell_exp_df[args.cell_level] == cell_type]
        ...
        # Calculate metrics     
        expressing_cells = cell_exp_df[cell_exp_df[args.gene] > 0].count()
        total_cells = cell_exp_df.count() 
        mean_expression = cell_exp_df[args.gene].mean(numeric_only=True)
        percent_expressing = (expressing_cells / total_cells * 100) if total_cells > 0 else 0
    ...
    # For each cell type and region, calculate mean_expression and percent_expressing
