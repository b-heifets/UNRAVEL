...
def parse_args():
    ...
    reqs.add_argument('-y', '--y_input', help='path/y_axis_image_*.nii.gz (e.g., glob pattern[s] for cFos maps).', required=True, nargs='*', action=SM)
...
def main():
    ...
    # Collect image paths
    img_paths = match_files(args.y_input)
    if not img_paths:
