...
def parse_args():
    parser = argparse.ArgumentParser(description='For each cluster, load <ochann> apply <ochann_seg_ilastik_1> to measure mean IF in segmented voxels', formatter_class=RawTextHelpFormatter)
    ...
    parser.add_argument('-cn', '--chann_name', help='Channel name (e.g., iba1 or iba1_rb20). Default: ochann', default='ochann', metavar='')
    parser.add_argument('-s', '--seg_dir', help='Name of segmentation dir (e.g., iba1_seg_ilastik_1)', metavar='')
...
def main():
    ...
    with Live(progress):
        for sample in samples:
            ...
            # Define input path
            input_dir = Path(sample_path, "clusters", args.out_dir_name, f"{args.chann_name}_cropped").resolve()
            ...
            for cluster in clusters:
                ...
                # Load cropped image (e.g., iba1_rb20)
                cropped_img_path = Path(input_dir, f"{sample}_cluster_{cluster}.nii.gz")
                cropped_img = load_3D_img(cropped_img_path, return_res=False)

                # Load segmentation (e.g., iba1_seg_ilastik_1)
                output_dir_path = Path(input_dir.parent, f"{args.seg_dir}_cropped", "3D_counts", f"crop_{args.seg_dir}_{sample}_native_cluster_{cluster}_3dc") 
                seg_img_path = Path(output_dir_path, f"crop_{args.seg_dir}_{sample}_native_cluster_{cluster}.nii.gz")
                ...
                output = Path(output_dir_path,f"crop_{args.seg_dir}_{sample}_native_cluster_{cluster}_mean_IF_in_seg.txt") 
