...
For now run for all samples and list one cluster to process (e.g., '1').

Automatic flipping of montage tiles (so that clusters have uniform orientation) requires experiment_summary/sample_overview.csv (from find_clusters.sh or overview.sh) and running montage.sh for all samples. 
"
  exit 1
fi
...
if (( $y_max > 12000 )); then #This will scale down large image tiles if needed so that montages can still be made
  new_y_dim=$((12000/${#ochann_array[@]}))
else
  new_y_dim=$y_dim_ave
fi
new_x_dim_float=$(echo "scale=5 ; $ave_aspect_ratio*$new_y_dim" | bc)
new_x_dim=$(echo "($new_x_dim_float+0.5)/1" | bc) #rounds
...
##################################################################
conditions=($(awk -F, '{print $4}' $exp_summary_dir/sample_overview.csv | uniq -c | awk 'NR>1{print $2}'))

columnar_montage () {
for i in ${!conditions[@]}; do 
  condition_sample_nums=($(grep ",${conditions[$i]}," $exp_summary_dir/sample_overview.csv | cut -d',' -f1))
  condition_sample_array=("${condition_sample_nums[@]/#/sample}")
  image_array=($(for s in ${condition_sample_array[@]}; do for c in $(eval echo "$clusters_to_process"); do echo "$prefix"_$1_"$s"_"$suffix"_"$c".tif ; done ; done))
  for c in $(eval echo "$clusters_to_process"); do
    montage ${image_array[@]} -resize "$new_x_dim"x"$new_y_dim"\! -geometry $border_size -tile 1x${#image_array[@]} -background white $1_montage_cluster_"$c"_${conditions[i]}.tif  
  done #https://stackoverflow.com/questions/1335815/how-to-slice-an-array-in-bash
done 
}
...
if [ "$samples_to_process" != "all" ]; then 
  ...
  for i in ${!conditions[@]}; do 
    for c in $(eval echo "$clusters_to_process"); do
      /usr/local/miracl/depends/Fiji.app/ImageJ-linux64 --ij2 -macro draw_cluster_outline $orig_dir/stats_cropped/stats_thr_montage_cluster_"$c"_${conditions[i]}.tif#$orig_dir/ochann_cropped/ochann_montage_cluster_"$c"_${conditions[i]}.tif#$orig_dir/ochann_rb"$radius"_cropped/ochann_rb"$radius"_montage_cluster_"$c"_${conditions[i]}.tif#$consensusDir/"$consensus"_montage_cluster_"$c"_${conditions[i]}.tif

      montage $orig_dir/ochann_cropped/outline_ochann_montage_cluster_"$c"_${conditions[i]}.tif $orig_dir/ochann_rb"$radius"_cropped/outline_ochann_rb"$radius"_montage_cluster_"$c"_${conditions[i]}.tif $consensusDir/outline_"$consensus"_montage_cluster_"$c"_${conditions[i]}.tif -geometry $border_size -tile 3x1 -background white $orig_dir/montages/full_montage_w_outlines_cluster_"$c"_${conditions[i]}.tif 
    done 
  done
fi 
