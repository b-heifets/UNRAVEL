...
def parse_args():
    parser = argparse.ArgumentParser(description='Reorient an .nii.gz from LPS to PIR and zero out global position in sform and qform.', formatter_class=SuppressMetavar)
...
def reorient_image(input_image, output_image):
    # Reorient the image
    run_command(f"fslswapdim {input_image} y -z -x {output_image}")
