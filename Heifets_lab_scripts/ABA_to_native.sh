...
ABA_to_native.sh <xy voxel size (um) or m (metadata)> <z voxel size or m>

Inputs:
reg_final/gubra_ano_split_10um_clar_vox.nii.gz

Outputs: 
sample??/reg_final/"$sample"_native_gubra_ano_split.nii.gz
'
  exit 1
fi
...
########################################
####### Atlas to full res native #######
########################################
if [ ! -f $output_dir/"$sample"_native_gubra_ano_split.nii.gz ]; then 

  echo " " ; echo "  Making $output_dir/"$sample"_native_gubra_ano_split.nii.gz " ; echo Start: $(date) ; echo " " 

  #Scale native atlas to full res
  ...
  mv $folder_w_warped_image/native_$image.nii $output_dir/"$sample"_native_gubra_ano_split.nii
  gzip -f -9 $output_dir/"$sample"_native_gubra_ano_split.nii #nii to nii.gz

  echo "  Made full res $output_dir/"$sample"_native_gubra_ano_split.nii.gz" ; echo End: $(date) ; echo " " 
else 
  echo " " ; echo "  Full res $output_dir/"$sample"_native_gubra_ano_split.nii.gz exists, skipping" ; echo " " 
fi
