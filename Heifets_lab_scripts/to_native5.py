...
from unravel_img_io import load_3D_img, load_image_metadata_from_txt, save_as_zarr, save_as_nii
...
Usage: to_native5.py -m <path/image_to_warp_from_atlas_space.nii.gz> -f <path/fixed_image.nii.gz> [-l]
...
@print_func_name_args_times()
def warp_to_native(moving_img_path, fixed_img_path, transforms_dir, reg_output_prefix, reg_input_res, fixed_img_res, metadata_path, legacy=False):
    ...
    native_img = scale_to_full_res(cropped_img, original_dimensions, zoom_order=args.zoom_order)
    return native_img
