...
echo " " ; echo "Running  native_clusters_any_immunofluor_rater_abc_dr.sh $1 $2 $3 $4 $5 $6  from $PWD at $(date)" ; echo " " 

orig_dir=$PWD
sample=${PWD##*/}
path_native_index=$(echo $1 | sed "s/['\"]//g")
output_folder=${path_native_index%/*/*}
#clusters_to_process=$(echo $4 | sed "s/['\"]//g") #ABC Add
#clusters=$(eval echo "$clusters_to_process") #ABC Add
...
python3 /usr/local/miracl/miracl/seg/native_clusters_any_immunofluor_rater_abc.py $1 $xy_res $z_res $output_folder $sample $seg_nifti $4 ########## edited pos arg to accomodate consensus or rater specific segmentations
