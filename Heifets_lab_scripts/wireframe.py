...
def parse_args():
    parser = argparse.ArgumentParser(description='Generate a thin wireframe image from an atlas NIfTI file.')
    parser.add_argument('-i', '--input', help='path/atlas_img.nii.gz', required=True, metavar='')
    parser.add_argument('-wo', '--wire_output', help='Wireframe image output path. Default: path/atlas_img_W.nii.gz', metavar='')
    parser.add_argument('-id', '--id_output', help='Wireframe image with atlas IDs output path. Default: path/atlas_img_W_IDs.nii.gz', metavar='')
