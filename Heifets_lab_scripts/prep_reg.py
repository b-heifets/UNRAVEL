...
def parse_args():
    ...
    parser.add_argument('-o', '--output', help='Output file name. Default: autofl_*um.nii.gz', default=None, metavar='')
    ...
    parser.add_argument('-t', '--tif_dir', help='Name of folder with raw autofluo tifs. Use as image input if *.czi does not exist.', default=None, metavar='')
    ...
    parser.epilog = "Outputs: ./reg_input/autofl_*um.nii.gz and ./reg_input/autofl_*um_tifs/slice_????.tif series"
...
def main():
    ...
    decorated_resample_reorient = print_func_name_args_times(resample_reorient)
    ...
    with Live(progress):
        for sample in samples:
            ...
            if args.output == None:
                args.output = f"autofl_{args.res}um.nii.gz"
            if args.out_dir == None:
                args.out_dir = "reg_input"
            ...
            img_reoriented = decorated_resample_reorient(img, xy_res, z_res, args.res, zoom_order=args.zoom_order)

            # Save autofl image as tif series (for brain_mask.py)
            tif_dir_out = Path(sample, args.out_dir, os.path.splitext(args.output)[0] + "_tifs").resolve() # e.g., ./sample01/reg_input/autofl_50um_tifs
