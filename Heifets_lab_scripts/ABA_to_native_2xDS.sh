...
ABA_to_native_2xDS.sh <xy voxel size (um) or m (metadata)> <z voxel size or m>
...
########################################
if [ ! -f $output_dir/"$sample"_2xDS_native_gubra_ano_split.nii.gz ]; then 

  echo " " ; echo "  Making $output_dir/"$sample"_2xDS_native_gubra_ano_split.nii.gz " ; echo Start: $(date) ; echo " " 
  ...
  /usr/local/miracl/depends/Fiji.app/ImageJ-linux64 --ij2 -macro to_native_2xDS $PWD/$folder_w_warped_image/$image#$x_dim_10um#$y_dim_10um#$z_dim_10um#$xmin#$DS_atlas_x#$ymin#$DS_atlas_y#$zmin#$zmax#$tif_x_dim#$tif_y_dim#$tif_z_dim

  rm -f $folder_w_warped_image/$image 
  
  # Convert to .nii.gz and move
  mv $folder_w_warped_image/2xDS_native_$image.nii $output_dir/"$sample"_2xDS_native_gubra_ano_split.nii
  gzip -f -9 $output_dir/"$sample"_2xDS_native_gubra_ano_split.nii #nii to nii.gz

  echo "  Made 2xDS res $output_dir/"$sample"_2xDS_native_gubra_ano_split.nii.gz" ; echo End: $(date) ; echo " " 
else 
  echo " " ; echo "  2xDS res $output_dir/"$sample"_2xDS_native_gubra_ano_split.nii.gz exists, skipping" ; echo " " 
fi
