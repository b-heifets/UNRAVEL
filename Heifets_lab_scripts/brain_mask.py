...
def load_img_resample_reorient_save_nii(sample_dir, args=None):
    ...
    tif_input_dir = Path(sample_dir, "reg_input", args.tifs_dir)
    ...
    '--output_format', 'tif',
    '--output_filename_format', f'reg_input/{args.tifs_dir}_seg_ilastik/{{nickname}}.tif',
    tif_input_dir]

    print(f"  {cmd}")
    subprocess.run(cmd)

    print(f"\n  Output: [default bold]reg_input/{args.tifs_dir}_seg_ilastik/[/]")

@unrvl.print_cmd_and_times
def main():
    args = parse_args()

    seg_path = Path("reg_input", f"{args.tifs_dir}_seg_ilastik")
