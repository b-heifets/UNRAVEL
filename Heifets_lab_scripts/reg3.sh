...
Detailed command:
reg3.sh <orient code> <1=OB, 0=OB, or ./custom_template.nii.gz> <w, l or r> <warped label res in tissue space: 10/25/50> [leave blank to process all samples or enter sample?? separated by spaces]
...
echo " " ; echo "Running reg3.sh $@ from $PWD" ; echo " " 
...
for sample in ${sample_array[@]}; do
  ...
  if [ ! -f reg_final/gubra_ano_split_${4}um_clar_downsample.nii.gz ]; then 
    ...
    mkdir -p parameters
    echo reg3.sh $@ >> parameters/reg_parameters.txt
    ...
    if [ "$3" == "w" ]; then
      ...
      echo "  Positional arg 3 is not w, l, or r (run: reg3.sh help) "
    fi
