...
def parse_args():
    parser = argparse.ArgumentParser(description='For each cluster, load cropped <ochann>, cluster mask, and <*_seg_ilastik_1> and mul together to measure mean IF in segmented voxels', formatter_class=RawTextHelpFormatter)
    ...
    parser.add_argument('-c', '--clusters_dir', help='Path relative to sample?? with cluster data: clusters/<output_dir>', metavar='')
    parser.add_argument('-i', '--input_dir', help='Dir in <output_dir> with cropped immunofluo images', metavar='')
    parser.add_argument('-s', '--seg_dir', help='Dir in <output_dir> with cropped segmentation image', metavar='')
    parser.add_argument('-o', '--out_dir', help='Output dir prefix (e.g., th_seg_ilastik_3)', metavar='')
    parser.add_argument('-v', '--verbose', help='Enable verbose mode', action='store_true')
    parser.epilog = """
Example usage: native_clusters_mean_IF_in_seg2.py -c clusters/output_folder -i iba1_rb20_cropped -s iba1_seg_ilastik_1/sample03_iba1_seg_ilastik_1.nii.gz_cropped -o th_seg_ilastik_3 -v

-o glm_iba1_rb20_cbsMeth_v_meth_18000p_vox_p_tstat1_FDR0.2_MinCluster100 -cn iba1_rb20 -s iba1_seg_ilastik_1 -v

Run native_clusters_mean_IF_in_seg2.py from the experiment directory containing sample?? folders or a sample?? folder.
