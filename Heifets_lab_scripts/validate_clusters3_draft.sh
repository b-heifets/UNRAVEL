...
validate_clusters3.sh <./exp_or_sample_dir list> <seg (s), val (v), both> <./stats_map> <FDR q value or n> <l, r, both, or ./custom_mask> <min cluster size in voxels> <xy res in um> <z res> <IF label> <consensus or specific rater #s> <clusters to process> <shift_history> <path/region_mask>

This script aims to validate clusters of significant voxels (hot/cold spots) identified from voxel-wise stats (find_clusters.sh -> glm.sh). validate_clusters3.sh consists of two main phases (segmentation of cells in raw data and measuring cell density in clusters). See ilastik.sh help and consensus.sh help for how to set up phase 1 (ochann/tif series is required). For more info on phase 2 (requires output(s) from glm.sh, reg.sh, consensus.sh as well as 488/tifs), run: validate_clusters3.sh help  
...
Validation: 
  fdr.sh (voxel-wise or cluster-wise correction for multiple comparisons, respectively)
...
if [ $# -ne 0 ]; then #if positional args provided, then
  ...
  q_value=$4
  mask=$(echo $5 | sed "s/['\"]//g") # mask for cluster correction
  min_cluster_size=$6
  xy_res=$7
  z_res=$8
  immuno_label=$(echo ${9} | sed "s/['\"]//g")
  seg_type=$(echo ${10} | sed "s/['\"]//g")
  clusters_to_process=$(echo ${11} | sed "s/['\"]//g") ; clusters_to_process_inputs="'$clusters_to_process'"
  shift_history=${12}
  region_mask=${13} # optional: mask for rev_cluster_index (e.g., to process specific anatomical regions)
...
inputs="validate_clusters3.sh '${path_array[@]}' $seg_or_val $path_and_stats_map $q_value $mask $min_cluster_size $xy_res $z_res $immuno_label $seg_type $clusters_to_process_inputs $shift_history $region_mask" ; echo " " ; echo " " ; 
echo "Rerun script with: " ; echo " " ; 
echo "###############################################"
echo "$inputs" 
echo "###############################################"
echo " " ; echo " " ; echo "validate_clusters3.sh <./exp_or_sample_dir list> <seg (s), val (v), both> <./stats_map> <FDR q value or n> <l, r, both, or ./custom_mask> <min cluster size in voxels> <xy res in um> <z res> <IF label> <consensus or specific rater #s> <clusters to process> <shift_history> <path/region_mask>" ; echo " " ; mkdir -p rerun_validate_clusters ; echo $inputs > rerun_validate_clusters/rerun_validate_clusters_$(date +"%Y_%m_%d_%I_%M_%p") ; echo " " ; echo " " ; echo " " 
...
if [ "$seg_or_val" == "v" ] || [ "$seg_or_val" == "both" ] ; then 
  ...
  for s in ${samples[@]}; do
    ...
    native_clusters_any_immunofluor_rater_abc.sh $s/clusters/$output_folder/native_cluster_index/native_"$output_folder"_rev_cluster_index.nii.gz $xy_res $z_res "$clusters" $immuno_label $seg_type 
