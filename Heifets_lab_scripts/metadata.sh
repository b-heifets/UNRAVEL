...
if [ ! -f parameters/metadata ]; then 
  if ls *.czi 1> /dev/null 2>&1; then 
    /usr/local/miracl/depends/Fiji.app/ImageJ-linux64 --ij2 -macro metadata $PWD/$(ls *.czi) #Zeiss file type
    ...
    /usr/local/miracl/depends/Fiji.app/ImageJ-linux64 --ij2 -macro metadata $PWD/$first_tif
    cd ..
