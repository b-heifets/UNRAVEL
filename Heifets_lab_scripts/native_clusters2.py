...
def parse_args():
    ...
    parser.add_argument('-s', '--seg', help='Dir name for segmentation image (e.g., cfos_seg_ilastik_1) or rel_path/seg_img', required=True, action=SM)
    ...
    parser.add_argument('-o', '--output', help='rel_path/clusters_info.csv', default=None, action=SM)
...
def main():
    ...
    with Live(progress):
        for sample in samples:
            ...
            native_cluster_index = warp_to_native(args.moving_img, args.fixed_img, args.transforms, args.reg_o_prefix, args.reg_res, args.fixed_res, args.metadata, args.legacy)
            ...
            # Lower bit-depth if possible
            if native_cluster_index.max() < 256:
                native_cluster_index = native_cluster_index.astype(np.uint8)
            elif native_cluster_index.max() < 65536:
                native_cluster_index = native_cluster_index.astype(np.uint16)
