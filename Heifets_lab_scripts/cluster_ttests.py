...
def parse_args():
    parser = argparse.ArgumentParser(description='Load CSV w/ cluster densities and conduct t-tests')
    parser.add_argument('-i', '--input_csv', help='Path to CSV with cluster densities', required=True, metavar='')
    parser.add_argument('-ord', '--order', help='Group Order for plotting (must match condition in CSV)', required=True, nargs=2, metavar='')
    parser.add_argument('-l', '--labels', help='Group Labels in same order', nargs=2, default=None, metavar='')
    parser.add_argument('-c', '--colors', help="Colors (<color> or <hexcode>) for the bars in the same order as groups (Default: #dadada #ddeeff)", nargs=2, default=['#dadada', '#ddeeff'], metavar='')
    parser.add_argument('--symbol_color', help='Color (<color> or <hexcode>) for the symbols in swarmplot (Default: #bababa #abface)', nargs=2, default=['#bababa', '#abcdef'], metavar='')
    parser.add_argument('-ort', '--orient', help='Orientation of bar graphs (h: horizontal, v: vertical; Default: h)', choices=['h', 'v'], default='h',  metavar='')
    parser.add_argument('--output_type', choices=['pdf', 'png', 'svg'], default='pdf', help='Output file type (pdf, png, or svg)', metavar='')
