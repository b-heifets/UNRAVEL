...
def parse_args():
    ...
    parser.add_argument('-f', '--fixed_img', help='path/fixed_image.nii.gz', required=True, metavar='')
    parser.add_argument('-o', '--output', help='path/native_image.zarr', required=True, metavar='')
    parser.add_argument('-s', '--seg_img', help='path/segmentation_image.nii.gz (to get full res dims for scaling)', metavar='')
    parser.add_argument('-t', '--tif_dir', help='Name of directory with full res tifs (alt way to get dims)', default='488', metavar='')
...
def scale_to_full_res(ndarray, new_width, new_height, new_depth, zoom_order=0):
    ...
    scaled_data = zoom(ndarray, zoom_factors, order=zoom_order)  # 'order=0' is for nearest-neighbor interpolation
    return scaled_data

@print_func_name_args_times()
def save_nii(img_path, output_path):
    nifti_img = nib.Nifti1Image(img_path, affine=np.eye(4))
...
@print_func_name_args_times()
def warp_to_native(img_path):
    # Load atlas space image
    atlas_space_ants_img = ants.image_read(args.moving_img)
    atlas_space_nib_img = nib.load(args.moving_img)
    data_type = atlas_space_nib_img.get_data_dtype()

    # Load fixed image
    fixed_ants_img = ants.image_read(args.fixed_img) # e.g., ./sample??/reg_final/clar_downsample_res25um.nii.gz

    # Warp from atlas to native space
    transforms_dir = Path(args.transforms).resolve()
    warped_ants_img = ants.apply_transforms(
        fixed=fixed_ants_img,
        moving=atlas_space_ants_img,
        transformlist=[
            f'{transforms_dir}/allen_clar_ants1Warp.nii.gz', 
            f'{transforms_dir}/allen_clar_ants0GenericAffine.mat',
            f'{transforms_dir}/init_tform.mat'
        ]
    )

    # Load images for calculating how much padding to remove
    reg_file_pre_padding = nii_to_ndarray(f'{args.transforms}/clar_res0.05_bias.nii.gz')
    reg_file_post_padding = nii_to_ndarray(f'{args.transforms}/clar_res0.05_pad.nii.gz')
    ...
    zf = round(args.reg_res / args.fixed_res)
    ...
    warped_img = warped_ants_img.numpy().astype(data_type) # convert to ndarray
    ...
    if args.seg_img: 
        nii_seg_img = nib.load(args.seg_img)
        ...
        tifs = Path(args.tif_dir).resolve().glob("*.tif") # Generator
    ...
    full_res_img = scale_to_full_res(reoriented_img, x_dim, y_dim, z_dim)

    # # Save the scaled image as .nii.gz
    # nifti_img = nib.Nifti1Image(full_res_img, affine=np.eye(4))
    # nib.save(nifti_img, args.output)

    # Save the scaled image as .zarr
    save_as_zarr(full_res_img, args.output)

    # print(f"Created full resolution image at {args.output}")
    # # else:
    # #     print(f"Full resolution image {args.output} already exists, skipping")



def main():
        
    print(f"\n    Warping {args.moving_img} to native space\n")
