...
if [ "$seg_or_val" == "v" ] || [ "$seg_or_val" == "both" ] ; then 
  ...
  for s in ${samples[@]}; do
    ...
    for i in $(eval echo "$clusters_to_process"); do
      ...
      ### 3d_count_cluster2_any_immunofluor_rater_abc_fibers.sh $s/clusters/$output_folder/"$seg_folder"_cropped/3D_counts/crop_"$seg_folder"_"${s##*/}"_native_cluster_"$i"_3dc/crop_"$seg_folder"_"${s##*/}"_native_cluster_"$i".nii.gz $i n
    done
