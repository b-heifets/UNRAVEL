...
def parse_args():
    parser = argparse.ArgumentParser(description='Load .nii.gz images matching pattern, get mean intensity of voxels in mask, and check for outliers (>3*SD +/- the mean)')
    parser.add_argument('-p', '--pattern', help='Regex pattern in quotes for matching .nii.gz images', default=None, metavar='')
    parser.add_argument('-m', '--mask', help='path/mask.nii.gz', default=None, metavar='')
    parser.add_argument('-o', '--output', help='path/name.[pdf/png]. Default: means_in_mask.pdf ', default='means_in_mask.pdf', metavar='')
