...
cp_prior_clusters.sh (to avoid redundant processing, relevant prior data copied to new cluster analysis folder if using same p value thresh and new min cluster size)
cluster_masks.sh (makes binary masks for each cluster at full res from native rev_cluster_index)
bounding_box.sh (determines location/extent of cluster for cropping) 
crop_cluster.sh (crops data to bounding box of cluster for ochann, ochann_rb*, [ABA]consensus, thresholded stats, & ABAcluster data, speeding up cell counting/cluster volume measurements and making of montage tiles)
ABAcluster.sh (binary clsuter mask multiplied by the ABA intensities)
ABAcluster_volumes.sh (region specific cluster volumes in the form of a CSV with a histogram of voxel counts for each intensity for volume calculations)
...
if [ "$seg_or_val" == "v" ] || [ "$seg_or_val" == "both" ] ; then 
  ...
  for s in ${samples[@]}; do
    ...
    # If using same FDR q and new min cluster size, copy prior data meeting criteria to new output folder to avoid redundant processing
    if [ "$q_value" != "n" ]; then cp_prior_clusters.sh "${stats_map::-7}"_FDR"$q_value"_MinCluster ${path_and_stats_map%/*}/"$output_folder" ; fi

    # Generate clusters masks, ./bounding_boxes/"${image::-7}"_fslstats_w.txt, & cropped clusters
    for i in $(eval echo "$clusters_to_process"); do
      cluster_masks.sh $s/clusters/$output_folder/native_cluster_index/native_"$output_folder"_rev_cluster_index.nii.gz $s/clusters/$output_folder/cluster_masks/"${s##*/}"_native_cluster_"$i".nii.gz $i 
      bounding_box.sh $s/clusters/$output_folder/cluster_masks/"${s##*/}"_native_cluster_"$i".nii.gz
      crop_cluster.sh $s/clusters/$output_folder/bounding_boxes/"${s##*/}"_native_cluster_"$i"_fslstats_w.txt clusters $s/clusters/$output_folder/cluster_masks/"${s##*/}"_native_cluster_"$i".nii.gz
    done 
    ...
    if [ "$regional_counts" == "y" ]; then 
      ...
      for i in $(eval echo "$clusters_to_process"); do
        crop_cluster.sh $s/clusters/$output_folder/bounding_boxes/"${s##*/}"_native_cluster_"$i"_fslstats_w.txt consensus $s/consensus/"${s##*/}"_consensus.nii.gz 
      done
    fi 
    ...
    for i in $(eval echo "$clusters_to_process"); do
      3d_count_cluster.sh $s/clusters/$output_folder/cluster_masks/"${s##*/}"_native_cluster_"$i".nii.gz $i $regional_counts
    done
  ...
  cd $exp_summary_dir/cluster_validation_summary/$output_folder
  cluster_densities.sh all "$clusters_to_process"
...
#Daniel Ryskamp Rijsketic 05/16/2022-06/21/2022 07/27/22-07/29/22 10/19-21/22 & Austen Casey (helped w/ summary scripts 8/2/22) (Heifets lab)
