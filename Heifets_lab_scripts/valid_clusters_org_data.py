...
def parse_args():
    ...
    parser.add_argument('-vd', '--vstats_dir', help='path/vstats_dir (vstats.py dir)', default=None, action=SM)
...
def copy_stats_files(item, new_dir, vstats_dir, p_val_txt):
    """Process statistical files associated with cluster corrections."""
    vstats_path = Path(vstats_dir)
    if vstats_path.exists():
        rev_cluster_index = next(vstats_path.glob(f"{item.name}.nii.gz"), None)
        if rev_cluster_index:
            cluster_correction_dir = rev_cluster_index.parent
...
def organize_validation_data(sample_path, clusters_path, validation_dir_pattern, density_type, target_dir, vstats_dir, p_val_txt):
    ...
    for item in clusters_path.glob(validation_dir_pattern):
        if item.is_dir():
            ...
            if vstats_dir:
                copy_stats_files(item, new_dir, vstats_dir, p_val_txt)
    ...
    with Live(progress):
        for sample in samples:
            ...
            if clusters_path.exists():
                organize_validation_data(sample_path, clusters_path, args.input, args.density_type, target_dir, args.vstats_dir, args.p_val_txt)
